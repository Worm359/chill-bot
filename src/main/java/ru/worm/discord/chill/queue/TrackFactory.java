...
//    }
    ...
    public List<Track> obtainTracks(String playlistId) {
        if (TextUtil.isEmpty(playlistId)) return Collections.emptyList();
        try {
            List<PlaylistItem> playlistItems = playlist(playlistId);
            if (playlistItems == null || playlistItems.isEmpty()) {
                log.error("failed to load playlist {}", playlistId);
    ...
    /**
     * Searches for track inside playlist first.<br>
     * Uses Youtube API to load the info, if not found.
     * @param youtubeUrl - youtube video url
     */
    public Optional<Track> obtainTrack(String youtubeUrl) {
        String videoId = YoutubeUtil.stripVideoUrl(youtubeUrl).orElse(null);
        ...
        return trackAlreadyPresent
                .or(() -> {
                    try {
                        ...
                        return Optional.of(new Track(videoId, title, duration));
                    } catch (IOException e) {
                        log.error(ExceptionUtils.getStackTrace(e));
                        return Optional.empty();
                    }
